...
- scheduler removal prevents BullMQ from emitting future occurrences, but an occurrence BullMQ already emitted into waiting, prioritized, or active work may still execute. Scheduling the same class and method later recreates that identity. Inventory queues are read independently rather than as one atomic snapshot, so concurrent changes or aliased origins can produce mixed-time duplicates or omissions; any rejecting read rejects the whole inventory call rather than returning partial data.
